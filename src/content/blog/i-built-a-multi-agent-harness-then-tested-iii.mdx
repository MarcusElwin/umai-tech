...
That's a good engineering shape.
...
iii isn't entering an empty field — every serious agent stack already answers the harness question somewhere. LangGraph checkpoints graph state inside your process.[^langgraph] Microsoft folded AutoGen and Semantic Kernel into one agent framework with an actor-style runtime.[^msaf] OpenAI's Agents SDK keeps the loop thin and in-process,[^openai-agents] while Temporal ships an official integration that turns that same loop into durable workflow code.[^temporal] Anthropic packages its own coding harness — loop, tools, permissions, subagents — as the Claude Agent SDK.[^claude-agent-sdk] OpenHands wraps each coding session in a sandboxed per-session runtime.[^openhands] Restate, Inngest, and Hatchet sell durable execution that agent builders increasingly borrow.[^restate][^inngest][^hatchet]
