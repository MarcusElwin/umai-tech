...
import PlaybookCards from '../../components/PlaybookCards.astro';
...
<PlaybookCards
  ...
  subtitle="Five principles, with what to do, what to avoid, and how it's already playing out in the wild. Copy the checklist below before your next planning meeting."
  principles={[
    {
      number: 1,
      title: 'Own the workflow, not the output',
      oneLine: 'The model produces tokens. The workflow produces switching cost.',
      doThis: 'Embed where ripping you out means ripping out the work — EHR, CRM, checkout, document workflow.',
      dontDoThat: 'Ship a chat UI on top of someone else\'s system of record and call it a product.',
      example: 'Procore in construction. Toast in restaurants. Veeva in life sciences. 300% pricing, under 3% churn.',
    },
    {
      number: 2,
      title: 'Build the harness first',
      oneLine: 'Decouple probabilistic reasoning from deterministic execution from day one.',
      doThis: 'Eval suite, memory tiers, policy gateway, idempotent tool dispatch. R.E.S.T. as the minimum bar.',
      dontDoThat: 'Treat orchestration, guardrails, and observability as "later" work to bolt on after launch.',
      example: 'Cursor at $50B uses the same models as everyone else. The harness is the differentiator.',
    },
    {
      number: 3,
      title: 'Spin the flywheel from day one',
      oneLine: 'Every interaction has to produce proprietary data, not just a response.',
      doThis: 'Wire HITL corrections, eval traces, and edge cases into the v1 data pipeline.',
      dontDoThat: 'Wait until you have scale to "add the data loop later" — by then competitors are 12 months ahead.',
      example: 'NBIM: 213K analyst hours/year saved by harness-captured corrections feeding tuned outputs.',
    },
    {
      number: 4,
      title: 'Compete on integration, not intelligence',
      oneLine: 'Your fight is on a different axis from the model labs\' benchmarks.',
      doThis: 'Go deep on the systems customers already run, the regulators they answer to, the workflows they live in.',
      dontDoThat: 'Bet on having a marginally better foundation model than your competitor.',
      example: 'Harvey + iManage + LexisNexis. Legora firmwide at Bird & Bird, Cleary, Linklaters, BCLP.',
    },
    {
      number: 5,
      title: 'Price for outcomes, not seats',
      oneLine: 'Value lives in the labor line of your customer\'s P&L. Price against that.',
      doThis: 'Tie pricing to outcomes — hours saved, fraud blocked, cycles compressed, errors prevented.',
      dontDoThat: 'Charge per login and watch your revenue cap at exactly the moment AI removes the need for more seats.',
      example: 'Stripe Radar: $917M fraud blocked / BFCM 2024. AIG: underwriting 5× faster, 75%→90%+ accuracy.',
    },
  ]}
/>
...
[^27]: Hugging Face FineWeb team, "The Synthetic Data Playbook: Generating Trillions of the Finest Tokens" (huggingface.co/spaces/HuggingFaceFW/finephrase); see also Cosmopedia (huggingface.co/blog/cosmopedia), 25B synthetic tokens for pre-training via Mixtral-8x7B