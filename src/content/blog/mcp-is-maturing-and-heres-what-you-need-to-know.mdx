...
Don't take my word for it. The repo ships a `./scripts/smoke.sh` that drives the whole thing — MCP, ACP, and UCP — against the live feed, piped through `jq`. Here's a real run:
...
One server, one feed, three protocols: MCP tools (steps 1–5), an ACP product feed plus checkout session (6–7), and UCP JSON-RPC (8) — the elicitation and step-up-auth challenge (steps 3–4) are the exact responses shown above, straight off the live catalog.
